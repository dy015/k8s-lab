...
reset_from_baseline() {
    log_step 1 "Restoring from baseline (removing resource limits)"

    # Remove resource limits to return to baseline (no limits)
    kubectl patch deployment ${DEPLOYMENT} -n ${NAMESPACE} --type='json' -p='[
      {
        "op": "remove",
        "path": "/spec/template/spec/containers/0/resources"
      }
    ]' 2>/dev/null || log_info "Resources already at baseline"

    log_success "Restored to baseline (no resource limits)"
    return 0
}
...
main() {
    ...
    log_section "Exercise 06: Reset to Baseline"
    ...
    # Force rollout restart to ensure clean state
    ...
    kubectl get deployment ${DEPLOYMENT} -n ${NAMESPACE} -o jsonpath='{.spec.template.spec.containers[0].resources}' || echo "No resource limits set (baseline)"
    echo
    echo
...
${BOLD}Ready for next exercise:${NC}
  ${CYAN}cd ../07-liveness-probe-fail${NC}
